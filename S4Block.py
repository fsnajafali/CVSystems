...
class S4Decoder(nn.Module):
    def __init__(self, in_feats=512, out_feats=512):
        super().__init__()

        self.LN = nn.LayerNorm(in_feats)
        self.S4 = S4(d_model=in_feats, transposed=False)
    ...
    def forward(self, x):
        xs4 = self.S4(self.LN(x))
        xmlp = self.MLP(xs4)
        xskip = self.Linear(x)

        return xmlp + xskip

class S4DecoderPool(nn.Module):
    def __init__(self, in_feats=512, out_feats=512, kernel_size=(1,2,2), stride=(1,2,2)):
        super().__init__()

        self.LN = nn.LayerNorm(in_feats)
        self.S4 = S4(d_model=in_feats, transposed=False)

        self.pool1 = nn.MaxPool3d(kernel_size=kernel_size, stride=stride)
        self.pool2 = nn.MaxPool3d(kernel_size=kernel_size, stride=stride)

        self.mlp1 = nn.Linear(in_feats, in_feats)
        self.mlp2 = nn.Linear(in_feats, out_feats)
    ...
    def forward(self, x):
        ...
        x_ln = self.LN(x)
        x_ln = rearrange(x_ln, 'bs f h w ch -> bs (f h w) ch')

        xs4 = self.S4(x_ln)
        xs4 = rearrange(xs4, 'bs (f h w) ch -> bs ch f h w', f=f, h=h, w=w)
        xpool1 = self.pool1(xs4)

        xpool2 = rearrange(x, 'bs f h w ch -> bs ch f h w')
        xpool2 = self.pool2(xpool2)

        xpool1 = rearrange(xpool1, 'bs ch f h w -> bs f h w ch')
        xpool2 = rearrange(xpool2, 'bs ch f h w -> bs f h w ch')

        xmlp = F.relu(self.mlp2(F.relu(self.mlp1(xpool1))))
        xskip = F.relu(self.fc(xpool2))
...
if __name__ == '__main__':
    ...
    S4_block = S4(d_model=512, transposed=False)
    #model = S4Decoder()
    x = torch.rand(5, 64, 512)
    x = S4_block(x)
    print(x.shape)