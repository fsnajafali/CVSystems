...
class RepNet(nn.Module):
    ...
    def forward(self, x, ret_sims = False):
        batch_size, c, _, h, w = x.shape
        x = rearrange(x, 'bs c l h w -> (bs l) c h w')
        #x = x.view(-1, c, h, w)
        x = self.resnetBase(x)
        x = x.view(batch_size, self.num_frames, x.shape[1],  x.shape[2],  x.shape[3])
        x = x.transpose(1, 2)
        #print("X going into that conv3D looks like this", x.shape)
        x = F.relu(self.bn1(self.conv3D(x)))
        #print("ON its way out it looks like", x.shape)
...
if __name__ == '__main__':
    x = torch.rand(5, 3, 64, 112, 112).cuda()
    model = RepNet(num_frames=64).cuda()
    x, _ = model(x)
    print(x.shape)